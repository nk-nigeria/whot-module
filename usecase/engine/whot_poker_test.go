...
func TestNewGameAndDeal(t *testing.T) {
	...
	assert.Equal(t, expectedCardCount, len(state.Cards["user1"].WhotCards))
	assert.Equal(t, expectedCardCount, len(state.Cards["user2"].WhotCards))
...
func TestPlayCard_Normal(t *testing.T) {
	...
	state.Cards["user1"] = &pb.ListCard{
		WhotCards: []*pb.WhotCard{
			{Rank: pb.WhotCardRank_WHOT_RANK_5, Suit: pb.WhotCardSuit_WHOT_SUIT_CIRCLE},
			{Rank: pb.WhotCardRank_WHOT_RANK_3, Suit: pb.WhotCardSuit_WHOT_SUIT_STAR},
		},
	}

	// Thiết lập bài trên bàn và lượt chơi
	state.TopCard = &pb.WhotCard{Rank: pb.WhotCardRank_WHOT_RANK_5, Suit: pb.WhotCardSuit_WHOT_SUIT_TRIANGLE}
	state.CurrentTurn = "user1"

	// Đánh lá cùng số (5)
	cardToPlay := &pb.WhotCard{Rank: pb.WhotCardRank_WHOT_RANK_5, Suit: pb.WhotCardSuit_WHOT_SUIT_CIRCLE}
	effect, err := engine.PlayCard(state, "user1", cardToPlay)

	// Kiểm tra kết quả
	assert.NoError(t, err)
	assert.Equal(t, entity.EffectPickThree, effect)
	assert.Equal(t, cardToPlay, state.TopCard)
	assert.Equal(t, 1, len(state.Cards["user1"].WhotCards))
}
...
func TestPlayCard_HoldOn(t *testing.T) {
	...
	state.Cards["user1"] = &pb.ListCard{
		WhotCards: []*pb.WhotCard{
			{Rank: pb.WhotCardRank_WHOT_RANK_1, Suit: pb.WhotCardSuit_WHOT_SUIT_CIRCLE}, // Hold On
			{Rank: pb.WhotCardRank_WHOT_RANK_3, Suit: pb.WhotCardSuit_WHOT_SUIT_STAR},
		},
	}

	// Thiết lập bài trên bàn và lượt chơi
	state.TopCard = &pb.WhotCard{Rank: pb.WhotCardRank_WHOT_RANK_1, Suit: pb.WhotCardSuit_WHOT_SUIT_TRIANGLE}
	state.CurrentTurn = "user1"

	// Đánh lá Hold On
	cardToPlay := &pb.WhotCard{Rank: pb.WhotCardRank_WHOT_RANK_1, Suit: pb.WhotCardSuit_WHOT_SUIT_CIRCLE}
	effect, err := engine.PlayCard(state, "user1", cardToPlay)
...
func TestPlayCard_PickTwo(t *testing.T) {
	...
	state.Cards["user1"] = &pb.ListCard{
		WhotCards: []*pb.WhotCard{
			{Rank: pb.WhotCardRank_WHOT_RANK_2, Suit: pb.WhotCardSuit_WHOT_SUIT_CIRCLE}, // Pick Two
			{Rank: pb.WhotCardRank_WHOT_RANK_3, Suit: pb.WhotCardSuit_WHOT_SUIT_STAR},
		},
	}

	// Thiết lập bài trên bàn và lượt chơi
	state.TopCard = &pb.WhotCard{Rank: pb.WhotCardRank_WHOT_RANK_2, Suit: pb.WhotCardSuit_WHOT_SUIT_TRIANGLE}
	state.CurrentTurn = "user1"

	// Đánh lá Pick Two
	cardToPlay := &pb.WhotCard{Rank: pb.WhotCardRank_WHOT_RANK_2, Suit: pb.WhotCardSuit_WHOT_SUIT_CIRCLE}
	effect, err := engine.PlayCard(state, "user1", cardToPlay)
...
func TestPlayCard_Whot(t *testing.T) {
	...
	state.Cards["user1"] = &pb.ListCard{
		WhotCards: []*pb.WhotCard{
			{Rank: pb.WhotCardRank_WHOT_RANK_20, Suit: pb.WhotCardSuit_WHOT_SUIT_UNSPECIFIED}, // Whot
			{Rank: pb.WhotCardRank_WHOT_RANK_3, Suit: pb.WhotCardSuit_WHOT_SUIT_STAR},
		},
	}
	...
	// Đánh lá Whot
	cardToPlay := &pb.WhotCard{Rank: pb.WhotCardRank_WHOT_RANK_20, Suit: pb.WhotCardSuit_WHOT_SUIT_UNSPECIFIED}
	effect, err := engine.PlayCard(state, "user1", cardToPlay)
...
func TestPlayCard_Invalid(t *testing.T) {
	...
	state.Cards["user1"] = &pb.ListCard{
		WhotCards: []*pb.WhotCard{
			{Rank: pb.WhotCardRank_WHOT_RANK_7, Suit: pb.WhotCardSuit_WHOT_SUIT_CIRCLE},
			{Rank: pb.WhotCardRank_WHOT_RANK_3, Suit: pb.WhotCardSuit_WHOT_SUIT_STAR},
		},
	}
	...
	cardToPlay := &pb.WhotCard{Rank: pb.WhotCardRank_WHOT_RANK_7, Suit: pb.WhotCardSuit_WHOT_SUIT_CIRCLE}
...
func TestDrawCardsFromDeck(t *testing.T) {
	...
	// Rút thêm 2 lá
	_, err := engine.DrawCardsFromDeck(state, "user1")

	// Kiểm tra kết quả
	assert.NoError(t, err)
	assert.Equal(t, initialCardCount+1, len(state.Cards["user1"].WhotCards))
}
...
func TestFinish_DirectWinner(t *testing.T) {
	...
	// Cài đặt bài trên tay người chơi
	state.Cards["user1"] = &pb.ListCard{WhotCards: []*pb.WhotCard{}} // user1 hết bài
	...
	assert.Equal(t, 2, len(result.ResultWhots)) // Có kết quả cho cả 2 người chơi

	// Kiểm tra người thắng
	var winnerFound bool
	for _, playerResult := range result.ResultWhots {
...
func TestFinish_ScoreCalculation(t *testing.T) {
	...
	state.Cards["user1"] = &pb.ListCard{
		WhotCards: []*pb.WhotCard{
			{Rank: pb.WhotCardRank_WHOT_RANK_5, Suit: pb.WhotCardSuit_WHOT_SUIT_CIRCLE}, // 5 điểm
			{Rank: pb.WhotCardRank_WHOT_RANK_3, Suit: pb.WhotCardSuit_WHOT_SUIT_STAR},   // 3*2 = 6 điểm
		},
	}
	...
	state.Cards["user2"] = &pb.ListCard{
		WhotCards: []*pb.WhotCard{
			{Rank: pb.WhotCardRank_WHOT_RANK_2, Suit: pb.WhotCardSuit_WHOT_SUIT_CIRCLE},    // 2 điểm
			{Rank: pb.WhotCardRank_WHOT_RANK_10, Suit: pb.WhotCardSuit_WHOT_SUIT_TRIANGLE}, // 10 điểm
		},
	}
